...
try {
    ...
    if (preg_match('#^users/(\d+)$#', $path, $m) && $method === 'DELETE') { $u = $adminOnly(); User::delete((int)$m[1]) ? ($json(['message'=>'Deleted']) && AuditLog::log($u['user_id'], 'DELETE', "Deleted user #{$m[1]}")) : $err('Failed to delete user'); exit; }
